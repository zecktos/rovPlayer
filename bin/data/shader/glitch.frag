...
float THRESHOLD = controlVal.x;
float mode = controlVal.y;
...
vec4 crapify(sampler2DRect sampler, vec2 uv, float pixelSize)
{
    return texture(sampler, uv*resolution - mod(uv, vec2(pixelSize) / resolution.xy));
}

vec3 edge(sampler2DRect sampler, vec2 uv, float sampleSize)
{
    float dx = sampleSize / resolution.x;
    float dy = sampleSize / resolution.y;
    return (
    mix(crapify(sampler, uv - vec2(dx, 0.0), sampleSize), crapify(sampler, uv + vec2(dx, 0.0), sampleSize), mod(uv.x, dx) / dx) +
    mix(crapify(sampler, uv - vec2(0.0, dy), sampleSize), crapify(sampler, uv + vec2(0.0, dy), sampleSize), mod(uv.y, dy) / dy)    
    ).rgb / 2.0 - texture(sampler, uv*resolution).rgb;
}

vec3 distort(sampler2DRect sampler, vec2 uv, float edgeSize)
{
    ...
    vec3 rgb = texture(sampler, uv*resolution + (distort + (pixel - pixel / 2.0) * vec2(shiftx, shifty) * (50.0 + 100.0 * (THRESHOLD*0.1))) * (THRESHOLD*0.1)).rgb;
...
void main()
{
	if (mode == 0.0)
	{
		...
		vec4 sample = texture( tex0, uv_nm*resolution);
	...
	else if (mode == 1.0)
	{
		...
		float rnd1 = rand( vec2(trunc( uv.x, 10.0*r0 ), my_time) );
		float r1 = 0.5 - 0.5 * gnm + rnd1;
		r1 = 1.0 - max( 0.0, ((r1<1.0) ? r1 : 0.9999999) );
		float rnd2 = rand( vec2(trunc( uv.y, 40.0*r1 ), my_time) );
		float r2 = sat( rnd2 );
		...
		for( int i=0; i<NUM_SAMPLES; ++i )
		{
			...
			uv.x = sat( uv.x + ofs * t );
			vec4 samplecol = texture( tex0, uv*resolution);
	...
	else if (mode == 2.0)
	{
	    vec2 uv = gl_FragCoord.xy / resolution.xy;
	    vec3 outColor;
	    outColor += distort(tex0, uv, 8.0);
	    fragColor = vec4(outColor, texture( tex0, uv*resolution ).a);
	}
}
